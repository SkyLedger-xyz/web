    ...
    <div>
      ...
      <ul {...getMenuProps()} className="relative" id="downshift" aria-labelledby="downshift">
        {status !== 'disabled' && isOpen && (options?.length > 0 || inputValue?.length > 0) ? (
          <div className="absolute z-10 mt-1 w-full rounded-lg border border-gray-200 bg-gray-100 shadow-xl shadow-gray-900/10">
            <div className="max-h-52 overflow-y-auto">
              {options.map((option, index) => {
                const isSelectedItem = selectedItem?.name && option?.name && selectedItem?.name === option?.name;
                const isHighlighted = highlightedIndex === index;
                const classes = classNames('cursor-pointer', {
                  'bg-gray-150': isHighlighted || isSelectedItem,
                });
